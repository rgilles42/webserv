...
namespace Webserv
{
	...
	void	ServerEvent::read_event(void)
	{
		...
		logstream << "Server read event: " << this->sock.getAddress().getStrAddress() << ":" << this->sock.getAddress().getIntPort();
		this->logger << std::make_pair(this->logger.DEBUG, logstream.str())  << std::endl;
