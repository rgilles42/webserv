#include "../../includes/events/CGI.hpp"
...
//        add_env_var("SERVER_NAME", this->server.name());
		...
		add_env_var("SERVER_PROTOCOL", "HTTP/1.1");
//        add_env_var("SERVER_PORT", srv.port());
//        add_env_var("REQUEST_METHODS",this->request.getMethod().c_str());
...
//        add_env_var("QUERY_STRING", this->request.getQuery().c_str());
...
//        add_env_var("HTTP_COOKIE", this->request.cookie());   //BONUS
	...
	void    CGI::read_event()
	{
		...
		size_read = ::read(this->fd_out[0], buffer, 2048);
		if (size_read < 0)
		{
			throw readCGIFailed();
		}
	...
	void    CGI::write_event(void)
	{
		ssize_t	size_write = 0;
//		if (request.getMethode() == "POST"/* && bodySize > 0*/)
//		{
//			size_write = write(this->write_fd, request.getBody(), request.bodySize());
//		}
	}
